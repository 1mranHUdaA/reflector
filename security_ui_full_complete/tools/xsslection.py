...
xsslection.py
-------------
Improved reflected XSS scanner.

- Decodes encoded query strings (e.g. param%3dvalue)
- Filters common tracking/static parameters
- Skips static resources (.js, .png, etc.)
- Injects payload '"><xsslection>'
- Detects ONLY the exact '<xsslection>' in response (not encoded/escaped)
- Filters results: ONLY shows reflection if Content-Type is text/html, 
  suppressing results from text/javascript, application/json, etc.
- Saves results to reflected.txt
"""
...
def decode_query_params(raw_query: str):
    """Decode and parse query string (handles param%3dvalue cases)."""
    if not raw_query:
        return {}
...
def check_exact_marker_in_text(content: str, url: str, parameter: str, verbose=False):
    """Detect only the exact '<xsslection>' in HTML body."""
...
def test_reflection(url, parameter, verbose=False, reflected_urls=None):
    try:
        ...
        # --- ENHANCED FALSE POSITIVE SKIP ---
        # Skip static binary/media content (existing)
        skip_types = ["image/", "video/", "audio/", "font/"]
        
        # Also skip common non-HTML content types where XSS is not applicable (UPDATED)
        skip_types.extend([
            "text/plain", 
            "text/xml", 
            "application/json", 
            "application/xml", 
            "text/css",
            "text/javascript" # Added to suppress false positives
        ])

        # Check if the Content-Type header contains any of the skip strings
        if any(x in ctype for x in skip_types):
            if verbose:
                print(f"[-] [skip] False positive content-type detected: {ctype}")
            return
        # ------------------------------------
        ...
        is_reflected = check_exact_marker_in_text(text, url, parameter, verbose)
        
        # We only proceed to record if it's reflected AND it is HTML
        # Note: We still check for "text/html" explicitly for the true positive case
...
if __name__ == "__main__":
    ...
    main(verbose_flag)# your xsslection.py here
